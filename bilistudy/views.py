...
def course_list(request):
    """用户课程列表"""
    courses = UserCourse.objects.filter(user=request.user).select_related('video').prefetch_related('video__episodes').order_by('-add_time')
    ...
    for course in courses:
        ...
        # 检查是否有学习计划
        study_plan = StudyPlan.objects.filter(user_course=course, user=request.user).first()
        if study_plan:
            course.study_plan = study_plan
            course.has_study_plan = True
        else:
            course.study_plan = None
            course.has_study_plan = False
...
def study_plans(request):
    ...
    # 获取没有学习计划的课程
    planned_course_ids = plans.values_list('user_course_id', flat=True)
    courses_without_plan = UserCourse.objects.filter(
        user=request.user
    ).exclude(id__in=planned_course_ids).order_by('-add_time')
    ...
    return render(request, 'bilistudy/study_plans.html', {
        ...
        'completed_plans': completed_plans,
        'overdue_plans': overdue_plans,
        'courses_without_plan': courses_without_plan,
    })
...
def create_study_plan(request):
    ...
    try:
        ...
        # 检查是否已有计划
        existing_plan = StudyPlan.objects.filter(user_course=user_course, user=request.user).first()
        if existing_plan:
            return JsonResponse({
                'success': False,
                'error': '该课程已有学习计划'
        ...
        plan_data = {
            'id': study_plan.id,
            'course_id': user_course.id,
            'course_title': user_course.custom_title or user_course.video.title,
            'total_days': study_plan.total_days,
            'daily_minutes': study_plan.daily_minutes,
            'focus_modules': study_plan.focus_modules,
            'start_date': study_plan.start_date.strftime('%Y-%m-%d'),
            'expected_end_date': study_plan.expected_end_date.strftime('%Y-%m-%d'),
            'progress_percentage': study_plan.progress_percentage,
            'days_passed': study_plan.days_passed,
            'is_overdue': study_plan.is_overdue,
        }
...
def reset_password_request(request):
    """请求密码重置（修改为简单的重置功能，只需要用户名/邮箱和原密码）"""
    try:
        ...
        if not all([username_or_email, old_password, new_password, confirm_password]):
            return JsonResponse({
                'success': False,
                'message': '请填写所有字段'
            })
